...
describe('RecordingsLibrary', () => {
  it('resolves audio only inside the configured recordings root', () => {
    ...
    expect(library.resolveRecordingAudio('2026-07-27T01-03-28-361Z')).toBe(
      '/library/2026-07-27T01-03-28-361Z/audio.wav',
    );
    expect(() => library.resolveRecordingAudio('../secrets')).toThrow('INVALID_RECORDING_ID');
  });

  it('resolves transcripts only from a validated recording directory', () => {
    const library = new RecordingsLibrary(() => '/library');

    expect(library.resolveRecordingTranscript('2026-07-27T01-03-28-361Z')).toBe(
      '/library/2026-07-27T01-03-28-361Z/transcript.json',
    );
    expect(() => library.resolveRecordingTranscript('/private/secret/transcript.json')).toThrow(
  ...
  it('does not mark an empty transcript artifact as ready', () => {
    ...
    expect(hasTranscriptSegments([{ start: 0, end: 1, text: 'Hello', speaker: 'You' }])).toBe(true);
  });
  ...
  it('shows only valid legacy, complete, and interrupted recording directories', async () => {
    ...
    await create('2026-08-12T01-02-03-006Z', {
      ...valid,
      status: 'interrupted',
      interruptions: [
    ...
    await create('2026-08-12T01-02-03-009Z', {
      ...valid,
      status: 'interrupted',
      interruptions: [
        { channel: 'you', startMs: 0, endMs: 20, recovered: true, reason: 'format-limit' },
      ],
    });
    await create('2026-08-12T01-02-03-011Z', valid);
    await fs.unlink(path.join(root, '2026-08-12T01-02-03-011Z', 'audio.wav'));
    await fs.symlink(
      path.join(root, '2026-08-12T01-02-03-004Z', 'audio.wav'),
      path.join(root, '2026-08-12T01-02-03-011Z', 'audio.wav'),
    );
    ...
    await fs.writeFile(
      path.join(root, '2026-08-12T01-02-03-012Z', 'audio.wav'),
      Buffer.alloc(44 + 1_284),
    );
    await fs.mkdir(path.join(root, '.in-progress'));
    await fs.symlink(
      path.join(root, '2026-08-12T01-02-03-004Z'),
      path.join(root, '2026-08-12T01-02-03-010Z'),
    );
